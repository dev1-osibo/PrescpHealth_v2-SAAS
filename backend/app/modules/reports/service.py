...
Follows the same structural patterns as alerts/service.py:
...
from app.modules.reports.exceptions import ReportError
...
class ReportService:
    ...
    async def request_clinical_report(
        ...
        tracker = BackgroundTaskTracker(db=self.db, tenant_id=self.tenant_id)
        task_id = await tracker.create_task(
            task_type="clinical_pdf",
            params={"patient_id": str(patient_id), "sections": sections},
        )
    ...
    async def request_referral_report(
        ...
        task_id = await tracker.create_task(
            ...
            params={"patient_id": str(patient_id), "referring_physician": referring_physician},
        )
