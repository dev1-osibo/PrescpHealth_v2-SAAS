...
# Invalid MIME types (should always be rejected)
...
# File sizes within the 25 MB limit (keep small to avoid excessive memory use)
# We test boundary: sizes from 1 byte up to 1MB, plus near the boundary
valid_size_strategy = st.one_of(
    st.integers(min_value=1, max_value=1024 * 1024),  # 1 byte to 1 MB
    st.integers(
        min_value=MAX_FILE_SIZE_BYTES - 1024,
        max_value=MAX_FILE_SIZE_BYTES,
    ),  # Near the 25 MB boundary
)

# File sizes exceeding the 25 MB limit (capped to avoid excessive memory use)
invalid_size_strategy = st.integers(
    min_value=MAX_FILE_SIZE_BYTES + 1,
    max_value=MAX_FILE_SIZE_BYTES + 1024,  # Just over the limit (up to 1KB over)
)
...
class TestDocumentUploadValidation:
    ...
    Core invariants:
    1. Valid MIME + valid size → upload accepted
    2. Invalid MIME (any size) → InvalidMimeTypeError
    3. Valid MIME + oversized → FileSizeExceededError
    """
    ...
    async def test_property_valid_mime_and_size_accepted(
        ...
        The service should persist the document and return a Document
        object without raising any validation exception.
        """
    ...
    async def test_property_oversized_file_rejected(
        ...
        # Use a small buffer but lie about the size via a mock
        # (we don't want to allocate 75MB+ of real memory per test)
        file_data = b"\x00" * file_size
